    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态" clearable>
          <el-option
            v-for="dict in dict.type.manage_partition_status"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['manage:partitionInfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:partitionInfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:partitionInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['manage:partitionInfo:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList" :columns="columns"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="partitionInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" v-if="columns[0].visible" prop="id" />
        ...
        <template slot-scope="scope">
          <dict-tag :options="dict.type.manage_partition_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
...
<script>
import { listPartitionInfo, getPartitionInfo, delPartitionInfo, addPartitionInfo, updatePartitionInfo } from "@/api/manage/partitionInfo";

export default {
  name: "PartitionInfo",
  dicts: ['manage_partition_status'],
  data() {
    return {
      //表格展示列
      columns: [
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/partitionInfo/export',
      // 表单校验
      rules: {
        name: [
  ...
  methods: {
    ...
    getList() {
      ...
      listPartitionInfo(this.queryParams).then(response => {
        this.partitionInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getPartitionInfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updatePartitionInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPartitionInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除分区信息编号为"' + ids + '"的数据项？').then(function() {
        return delPartitionInfo(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `partitionInfo_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
