...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/violationInfo/export',
      // 表单校验
      rules: {
        libraryId: [
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateViolationInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addViolationInfo(this.form).then(response => {
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `violationInfo_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
