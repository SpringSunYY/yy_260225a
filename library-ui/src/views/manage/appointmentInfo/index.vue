    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:appointmentInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['manage:appointmentInfo:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listAppointmentInfo, getAppointmentInfo, delAppointmentInfo, addAppointmentInfo, updateAppointmentInfo } from "@/api/manage/appointmentInfo";
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/appointmentInfo/export',
      // 表单校验
      rules: {
        libraryId: [
  ...
  methods: {
    ...
    getList() {
      ...
      listAppointmentInfo(this.queryParams).then(response => {
        this.appointmentInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getAppointmentInfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateAppointmentInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAppointmentInfo(this.form).then(response => {
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `appointmentInfo_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
